...
using SQL8r.WinForms.NodeTypes.Project;
    ...
    public class ProjectPresenter : IPresenter, IViewPresenter<MainForm>
    {
        ...
        public void Init()
        {
           ...
           // _view = view;
            ...
            _view.tsbAddSqlFile.Click += tsbAddFile_Click;
            _view.tsbAddSqlFile.Tag = MainPresenter.FileTypes.SQL;
            _view.tsbAddSqlFile.Image = _view.TreeImageList.Images[15];
            _view.tsbAddSqlFile.ImageTransparentColor = Color.Transparent;

            _view.tsbAddCSharpFile.Click += tsbAddFile_Click;
            _view.tsbAddCSharpFile.Tag = MainPresenter.FileTypes.CSharp;
            _view.tsbAddCSharpFile.Image = _view.TreeImageList.Images[13];
            _view.tsbAddCSharpFile.ImageTransparentColor = Color.Transparent;

            _view.tsbAddVbNetFile.Click += tsbAddFile_Click;
            _view.tsbAddVbNetFile.Tag = MainPresenter.FileTypes.VBNET;
            _view.tsbAddVbNetFile.Image = _view.TreeImageList.Images[16];
            _view.tsbAddVbNetFile.ImageTransparentColor = Color.Transparent;
        ...
        public Image GetImageForExtension(string type)
        {
        	...
        	if (type.Equals(MainPresenter.FileTypes.CSharp))
        	{
        		return _view.TreeImageList.Images[13];
        	}
        	if (type.Equals(MainPresenter.FileTypes.VBNET))
        	{
        		return _view.TreeImageList.Images[16];
        	}
        	return _view.TreeImageList.Images[15];
        }
        ...
        private void tvwProject_ValidateLabelEdit(object sender, ValidateLabelEditEventArgs e)
        {
            ...
            else
            {
                ...
                else if (node is FolderNode)
                {
                    ...
                    SQL8rProjectHandler.Instance.RenameFolder(oldFileName, newFileName);
               ...
               //   string newRelFilename = newReldirname + @"\" + newName;
                 //   string oldRelFilename = newReldirname + @"\" + oldName;
                    ...
                    SQL8rProjectHandler.Instance.RenameFile(oldFileName, newFileName);
                }
        ...
        private void createFolder()
        {
            if (_view.tvwProject.SelectedNodes != null && _view.tvwProject.SelectedNodes.Count > 0)
            {
                ...
                if (node is ProjectNode)
                {
                    ...
                    foNode.BeginEdit();
                    SQL8rProjectHandler.Instance.AddFolder(newdir.FullName);                   
                }
                else if (node is FolderNode)
                {
                    ProjectNode pn = getProjectNode(node);
        ...
        private void createFile(string fileType)
        {
            ...
            else if (fileType == MainPresenter.FileTypes.CSharp)
            {
                typeSpecificFileName = "untitled.cs";
            }
            else if (fileType == MainPresenter.FileTypes.VBNET)
            {
                typeSpecificFileName = "untitled.vb";
            }
            ...
            if (_view.tvwProject.SelectedNodes != null && _view.tvwProject.SelectedNodes.Count > 0)
            {
                ...
                if (node is ProjectNode)
                {
                    ...
                    if (!file.Exists)
                    {
                        ...
                        node.Expanded = true;
                        SQL8rProjectHandler.Instance.AddFile(fiNode.GetFilePath());
                        fiNode.BeginEdit();
                    }
                    else
                    {
                        MessageBox.Show("File already exists in project");
                    }
                }
                else if (node is FolderNode)
                {
                 ...
                 //   string newRelFilename = newReldirname + @"\" + typeSpecificFileName;
                    ...
                    if (!file.Exists)
                    {
                        ...
                        node.Nodes.Add(fiNode);
                        node.Expanded = true;
                        SQL8rProjectHandler.Instance.AddFile(newdirname + @"\" + typeSpecificFileName);
                        fiNode.BeginEdit();
                    }
                    else
                    {
                        MessageBox.Show("File already exists in folder");
                    }
                }
            }
        }
        ...
        private void tsbRemoveFile_Click(object sender, EventArgs e)
        {
            if (_view.tvwProject.SelectedNodes.Count > 0)
            {
                ...
                if (fileNode != null)
                {                    
                    SQL8rProjectHandler.Instance.RemoveFile(fileNode.GetFilePath());
                ...
                else
                {
                    ...
                    if (folderNode != null)
                    {
                        SQL8rProjectHandler.Instance.RemoveFolder(folderNode.GetFolderPath());
                        _view.tvwProject.SelectedNodes[0].Remove();
                    }
                }
            }
        ...
        private void tsbNewProject_Click(object sender, EventArgs e)
        {
            ...
            {
                Sql8rProject pro = SQL8rProjectHandler.Instance.NewProject(sfd.FileName);
                if (pro != null)
                {
                    presentFiles(pro);
                }
        ...
        {
            Sql8rProject pro = SQL8rProjectHandler.Instance.LoadProject(fileName);
            presentFiles(pro);
        ...
        public void OpenMSBuildProject(string fileName)
        {
            Sql8rProject pro = MsBuildProjectHandler.Instance.LoadProject(fileName);
            pro.Name += ".sqlproj";
            pro = SQL8rProjectHandler.Instance.ImportProject(pro);
        ...
        private void presentFiles(Sql8rProject pro)
        {
        ...
        {
            if (filePath.ToUpperInvariant().EndsWith(".SQL", StringComparison.OrdinalIgnoreCase))
            {
                var p = (EditorWithDataGridPresenter)MainPresenter.Instance.OpenMdiTab(MainPresenter.MdiTabKeys.EditorWithDataGrid);
                p.OpenFile(filePath);
            }
            else if (filePath.ToUpperInvariant().EndsWith(".CS", StringComparison.OrdinalIgnoreCase) ||
                         filePath.ToUpperInvariant().EndsWith(".VB", StringComparison.OrdinalIgnoreCase))
            {
                var p = (EditorWithDataGridPresenter)MainPresenter.Instance.OpenMdiTab(MainPresenter.MdiTabKeys.EditorWithDataGrid);
                p.OpenFile(filePath);
            }
