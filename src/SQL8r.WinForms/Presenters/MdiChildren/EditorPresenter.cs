...
{
    public class EditorPresenter : IChildPresenter<EditorWithDataGridChildForm>
    {
...
//            _view.pasteToolStripCtxMenuItem.Click += tsbPaste_Click;
		...
		private void tsbCompileToFile_Click(object sender, EventArgs e)
		{
			...
			SaveFileDialog dlg = DialogFactory.Instance.GetCompileToFileDialog();
			if (dlg.ShowDialog() == DialogResult.OK)
			{
				string outFileName = dlg.FileName;

				compileCode(ext, code, outFileName);
			}
		}
        ...
        private void btnExecuteOnNewConnection_Click(object sender, EventArgs e)
        {
            ...
            if (p.ShowDialog() == DialogResult.OK)
            {
				executeQuery(p.GetConnection(), _view.tslFileName.Text);
            }
        }
