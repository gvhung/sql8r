...
namespace SQL8r.Logic.Odbc.Handlers
{
    ...
    [ServerVersionMetaData(ServerVersionId.ODBCAccess2003, "1.0", true)]
    public class OdbcAccessDatabaseHandler : IDatabaseManager
    {
        private ServerConnectionSettings _settings;
        public OdbcAccessDatabaseHandler()
        {
        }
        public OdbcAccessDatabaseHandler(ServerConnectionSettings settings)
        {
            _settings = settings;
        }
        ...
        public string ConnectionString
        {
            get
            {
                return string.Format("DRIVER={{Microsoft Access Driver (*.mdb)}};DBq={0};", _settings.DatabaseName);
            }
        }
